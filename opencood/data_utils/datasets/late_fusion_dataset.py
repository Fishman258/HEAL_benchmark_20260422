...
def getLateFusionDataset(cls):
    ...
    class LateFusionDataset(cls):
        ...
        def get_item_test(self, base_data_dict, idx):
            ...
            if 'ego' in processed_data_dict:
                processed_data_dict['ego'].update({
                    'sample_idx': int(idx),
                    'cav_id_list': cav_id_list_newname,
                    'record_len': np.asarray([len(cav_id_list_newname)], dtype=np.int64),
                    'lidar_pose': np.asarray(lidar_pose_list, dtype=np.float32),
                    'lidar_pose_clean': np.asarray(lidar_pose_clean_list, dtype=np.float32),
                })
        ...
        def collate_batch_test(self, batch):
            ...
            ego_source = batch.get('ego', {})
            if isinstance(ego_source, dict):
                sample_idx = ego_source.get('sample_idx')
                cav_id_list = ego_source.get('cav_id_list')
                record_len = ego_source.get('record_len')
                lidar_pose = ego_source.get('lidar_pose')
                lidar_pose_clean = ego_source.get('lidar_pose_clean')
                if sample_idx is not None and cav_id_list is not None and record_len is not None:
                    output_dict['ego'].update({
                        'sample_idx': int(sample_idx),
                        'cav_id_list': list(cav_id_list),
                        'record_len': torch.from_numpy(np.asarray(record_len, dtype=np.int64)),
                    })
                if lidar_pose is not None:
                    output_dict['ego']['lidar_pose'] = torch.from_numpy(
                        np.asarray(lidar_pose, dtype=np.float32)
                    )
                if lidar_pose_clean is not None:
                    output_dict['ego']['lidar_pose_clean'] = torch.from_numpy(
                        np.asarray(lidar_pose_clean, dtype=np.float32)
                    )
