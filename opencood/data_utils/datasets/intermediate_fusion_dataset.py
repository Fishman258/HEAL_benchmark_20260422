...
def getIntermediateFusionDataset(cls):
    ...
    class IntermediateFusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            # Some online pose-correction methods (e.g., lidar_reg / HKUST baselines)
            # require per-CAV raw point clouds. Historically we only exported those
            # under `visualize`, which makes benchmark semantics depend on a
            # visualization flag. Infer a dedicated export switch from pose_provider.
            pose_provider = params.get("pose_provider") if isinstance(params, dict) else None
            if not isinstance(pose_provider, dict):
                pose_provider = {}
            online_method = str(pose_provider.get("online_method") or "").lower().strip()
            self.export_lidar_np_by_cav = bool(
                pose_provider.get("enabled", False) and online_method == "lidar_reg"
            )
            ...
            force_pose_confidence = params.get("force_pose_confidence")
            try:
                self.force_pose_confidence = None if force_pose_confidence is None else float(force_pose_confidence)
            except Exception:
                self.force_pose_confidence = None
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            if self.load_lidar_file or self.visualize:
                ...
                # Keep per-CAV raw points for online LiDAR-registration pose correction.
                # Export local-frame points even when proj_first=True (which mutates lidar_np in-place).
                if self.visualize or self.export_lidar_np_by_cav:
                    lidar_np_for_reg = lidar_np.copy() if self.proj_first else lidar_np
                    selected_cav_processed.update({'lidar_np': lidar_np_for_reg})
        ...
        def __getitem__(self, idx):
            ...
            if self.pose_override_enabled:
                ...
                if self.pose_override_map:
                    ...
                    applied = True
                # Refresh the cached ego pose for downstream consumers.
            ...
            lidar_np_by_cav = []  # raw per-CAV points for online lidar_reg pose correction
            ...
            if self.force_pose_confidence is not None:
                for cav_id in cav_id_list:
                    cav_content = base_data_dict.get(cav_id)
                    if not isinstance(cav_content, dict):
                        continue
                    params = cav_content.get("params") or {}
                    if not isinstance(params, dict):
                        continue
                    params["pose_confidence"] = float(self.force_pose_confidence)
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                if (self.visualize or self.export_lidar_np_by_cav) and 'lidar_np' in selected_cav_processed:
                    lidar_np_by_cav.append(selected_cav_processed['lidar_np'])
            ...
            if (self.visualize or self.export_lidar_np_by_cav) and lidar_np_by_cav:
                # List aligned with cav_id_list / lidar_pose order. Left on CPU (NumPy)
                # so train_utils.to_device will not move it to GPU.
                processed_data_dict['ego'].update({'lidar_np_by_cav': lidar_np_by_cav})
        ...
        def collate_batch_train(self, batch):
            ...
            for i in range(len(batch)):
                ...
                if self.visualize:
                    ...
                    if 'lidar_np_by_cav' in ego_dict:
                        lidar_np_by_cav_list.append(ego_dict['lidar_np_by_cav'])
            ...
            if lidar_np_by_cav_list:
                flat = []
                for per in lidar_np_by_cav_list:
                    if isinstance(per, list):
                        flat.extend(per)
                if flat:
                    output_dict['ego'].update({'lidar_np_by_cav': flat})
