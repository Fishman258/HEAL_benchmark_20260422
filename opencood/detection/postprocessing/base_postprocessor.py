...
class BasePostprocessor(object):
    ...
    def generate_object_center_dairv2x_single_hetero(self,
        ...
        # GT generation must stay in the clean geometry contract.
        # Using the noisy runtime pose here makes DAIR labels drift with the
        # benchmark noise sweep, which contaminates AP even when fusion/runtime
        # geometry is later corrected.
        cav_coor = cav_content['params'].get('lidar_pose_clean',
                                             cav_content['params']['lidar_pose']) # T_world_cav
    ...
    def generate_visible_object_center(self,
        ...
        visibility_bgr = cav_contents[0].get("bev_visibility.png")
        visibility_map = None
        if visibility_bgr is not None:
            visibility_map = np.asarray(cv2.cvtColor(visibility_bgr, cv2.COLOR_BGR2GRAY))
        ...
        if visibility_map is None:
            # Fallback for datasets without `bev_visibility.png`: treat all objects as visible.
            ...
            for k, v in tmp_object_dict.items():
                if k in output_dict:
                    updated_tmp_object_dict[k] = v  # visible
            output_dict = {}
